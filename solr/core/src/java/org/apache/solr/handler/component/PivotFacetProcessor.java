  ...
  public SimpleOrderedMap<List<NamedList<Object>>> process(String[] pivots) throws IOException {
    ...
    for (String pivotList : pivots) {
      ...
      if(fieldValueList != null ){//We have terms params
        List<String> refinementValuesByField  = StrUtils.splitSmart(fieldValueList, ",", false);//All values, split by the field they should go to
        for( int i=refinementValuesByField.size()-1; i>0; i-- ) {
          vnames.push(refinementValuesByField.get(i));//Only for [1] and on
        }
        if(refinementValuesByField.size()>0){
          firstFieldsValues=refinementValuesByField.get(0);
        }

      }
      
      NamedList<Integer> superFacets;
      if(firstFieldsValues !=""){
        superFacets = this.getListedTermCounts(field,firstFieldsValues);
      } else {
        superFacets = this.getTermCountsForPivots(field);
      }
      //END REFINEMENT
  ...
  protected List<NamedList<Object>> doPivots(NamedList<Integer> superFacets,
    ...
    for (Map.Entry<String, Integer> kv : superFacets) {
      ...
      if (kv.getValue() >= minMatch) {  
        ...
        SimpleFacets sf = new SimpleFacets(req, subset, params);
        
        if( subField != null )  {
          NamedList<Integer> nl;
          if(!vnames.isEmpty()){ 
            nl = sf.getListedTermCounts(subField,vnames.pop());
          } else{
            nl = sf.getTermCountsForPivots(subField);
          }
